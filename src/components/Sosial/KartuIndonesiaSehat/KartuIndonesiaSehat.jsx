  ...
  const handleCreateSosialForm = async (data) => {
    ...
    if (KK) {
      ...
      const storageRef = ref(
        storage,
        `documents/sosial/kis/${KKSelected.name}`
      );
    ...
    if (KTP) {
      ...
      const storageRef = ref(
        storage,
        `documents/sosial/kis/${KTPSelected.name}`
      );
    ...
    if (suratRekomendasiRTRW) {
      ...
      const storageRef = ref(
        storage,
        `documents/sosial/kis/${suratRekomendasiRTRWSelected.name}`
      );
    ...
    const dataJSON = {
      formTypeId: "8be219d0-1c48-4b7f-bb9e-7aa44c34d613",
        ...
        <div className="mb-4 d-flex justify-content-between">
          <div>
            <h5 className="mb-1 text-heading-5 text-grey-1">
              Pengajuan Kartu Indonesia Sehat
            </h5>
...
export default KartuIndonesiaSehat;
