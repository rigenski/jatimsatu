  ...
  const handleCreateSosialForm = async (data) => {
    ...
    if (KK) {
      ...
      const storageRef = ref(
        storage,
        `documents/sosial/bpjs/${KKSelected.name}`
      );
    ...
    if (KTP) {
      ...
      const storageRef = ref(
        storage,
        `documents/sosial/bpjs/${KTPSelected.name}`
      );
    ...
    if (suratRekomendasiRTRW) {
      ...
      const storageRef = ref(
        storage,
        `documents/sosial/bpjs/${suratRekomendasiRTRWSelected.name}`
      );
    ...
    const dataJSON = {
      formTypeId: "ddbf5fa7-ad35-4a08-9b9a-a3d84fbb0c53",
